...
def main2(kw_str, filecontents):
    ...
    for index, row in final_data_frame1.iterrows():
        # col is referred by index of column array
        db_col_names = {'rid': " ", 'rDate': default_date, 'rDate2': default_date, 'rRating': 0.0, 'rText': " ", 'rTitle': " ",
                        'rURL': " ", 'rUser': " "}
        try:
            for dbc in db_col_names.keys():
                if dbc in df_col_names:
                    db_col_names[dbc] = row[dbc]
                    #print(db_col_names[dbc])

            obj = Uploads.objects.create(rid=db_col_names['rid'], rDate=db_col_names['rDate'], rDate2=db_col_names['rDate2'],
                                         rRating=db_col_names['rRating'], rText=db_col_names['rText'], rTitle=db_col_names['rTitle'],
                                         rURL=db_col_names['rURL'], rUser=db_col_names['rUser'], pCategory=kw_str)
            fk_rid_obj = row['rid']
